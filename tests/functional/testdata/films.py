ALL_FILM_WORKS = [
    {
        "uuid": "3f8873be-f6b1-4f3f-8a01-873924659851",
        "title": "Justin Bieber: A Star Was Born",
        "imdb_rating": 1.0
    },
    {
        "uuid": "b9151ead-cf2f-4e14-aeb9-c4617f68848f",
        "title": "Star Quest: The Odyssey",
        "imdb_rating": 1.5
    },
    {
        "uuid": "c69c0c98-1eef-4bc1-92da-192d7b637d58",
        "title": "Battle Star Wars",
        "imdb_rating": 1.6
    },
    {
        "uuid": "463d7234-2af1-4e54-a353-2a8c2c269103",
        "title": "Chatur Singh Two Star",
        "imdb_rating": 1.8
    },
    {
        "uuid": "a9bbc1d8-bb8a-41d2-b61a-d8ddc9b31ede",
        "title": "Amy Winehouse: Fallen Star",
        "imdb_rating": 1.8
    },
    {
        "uuid": "547147ed-d835-4c22-8c42-4a800d6f32b0",
        "title": "Star Force: Fugitive Alien II",
        "imdb_rating": 1.9
    },
    {
        "uuid": "f06315d4-eab6-4fdc-b768-7c046cedfd9b",
        "title": "Starmania. Österreich sucht den neuen Star",
        "imdb_rating": 2.1
    },
    {
        "uuid": "134989c3-3b20-4ae7-8092-3e8ad2333d59",
        "title": "The Star Wars Holiday Special",
        "imdb_rating": 2.1
    },
    {
        "uuid": "5c855467-9c2b-491d-a179-c217ea543e93",
        "title": "Porn Star Zombies",
        "imdb_rating": 2.4
    },
    {
        "uuid": "e60412c9-43d2-48c0-94a1-81c6b04af84e",
        "title": "Confessions of an Action Star",
        "imdb_rating": 2.4
    }
]


ONE_FILM_WORK = {
    "uuid": "5c855467-9c2b-491d-a179-c217ea543e93",
    "title": "Porn Star Zombies",
    "imdb_rating": 2.4,
    "description": "A group of adult film cast and crew members must discover "
                   "what is causing their fellow stars to rabidly attack them.",
    "directors": [
        {
            "id": "317a9da2-dffb-4052-93b7-05b796f5c726",
            "name": "Keith Emerson"
        }
    ],
    "actors": [
        {
            "id": "59d8eb04-b03a-4bd6-9b55-9914c2eb2abb",
            "name": "Josh Anderson"
        },
        {
            "id": "adf8f1cc-9825-4c38-84f4-75e868648fae",
            "name": "Jamey Green"
        },
        {
            "id": "317a9da2-dffb-4052-93b7-05b796f5c726",
            "name": "Keith Emerson"
        },
        {
            "id": "184bb84d-b293-4c95-a8f4-3a7e0e4ed0e9",
            "name": "Joshua Cameron"
        }
    ],
    "writers": [
        {
            "id": "317a9da2-dffb-4052-93b7-05b796f5c726",
            "name": "Keith Emerson"
        }
    ],
    "genres": [
        {
            "id": "f39d7b6d-aef2-40b1-aaf0-cf05e7048011",
            "name": "Horror"
        },
        {
            "id": "5373d043-3f41-4ea8-9947-4b746c601bbd",
            "name": "Comedy"
        }
    ]
}


FILM_NOT_FOUND = {
    'detail': 'Film was not found'
}


SEARCHED_FILM_WORKS = [
    {
        "uuid": "1e088341-97e1-4eff-a5b1-0e0113274394",
        "title": "Kin: Fallen Star",
        "imdb_rating": 5.1
    },
    {
        "uuid": "63b7be87-aef1-4433-9ba7-5f406b0c6c4d",
        "title": "The Lucky Star",
        "imdb_rating": 6.6
    },
    {
        "uuid": "e5974732-dce9-42ea-a26c-269e5047cb6d",
        "title": "Date My Porn Star",
        "imdb_rating": 5.9
    }
]
